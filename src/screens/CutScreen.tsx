      ...
      <main className="mx-auto max-w-[1200px] px-5 py-5 space-y-4">
        ...
        <div className={ui.panel}>
          <div className="px-4 py-3 border-b border-zinc-200 flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-extrabold text-zinc-900">Cantidades por producto</div>
              <div className="text-xs text-zinc-500">Esto es lo que usarás para el corte (producción).</div>
            </div>
          </div>
          ...
          <div className="p-4 overflow-auto">
            {!cutData ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {products.map((p, idx) => (
                    <tr key={`${p.name}-${idx}`} className="border-b border-zinc-100">
                      <td className="py-2 pr-2 font-extrabold text-zinc-900">{p.name}</td>
                      <td className="py-2 px-2 text-zinc-700">{p.category}</td>
                      <td className="py-2 px-2 text-right font-extrabold text-zinc-900">{safeNum(p.qty)}</td>
                      <td className="py-2 pl-2 text-right font-extrabold text-zinc-900">{money(safeNum(p.subtotal))}</td>
                    </tr>
                  ))}

                  <tr>
                    <td className="py-3 pr-2 font-extrabold text-zinc-900">TOTAL</td>
                    <td className="py-3 px-2 text-zinc-700">—</td>
                    <td className="py-3 px-2 text-right text-zinc-700">—</td>
                    <td className="py-3 pl-2 text-right font-extrabold text-zinc-900">{money(grandTotal)}</td>
                  </tr>
                </tbody>
              </table>
            )}
